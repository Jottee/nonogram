    ...
    public static void addMainPanels(JFrame puzzleFrame) {
        ...
        for (int m = 0; m < 3; m++) {
            for (int n = 0; n < 3; n++) {
                ...
                puzzlePanel.add(panelHolder[m][n]);
            }
        }
        panelHolder[0][1].setBackground(Color.BLUE);
        panelHolder[1][0].setBackground(Color.BLUE);
        panelHolder[1][1].add(new JPanel());


        createPuzzle(panelHolder[1][1], a, b);
    }

    public static void createPuzzle(JPanel puzzlePnl, int rows, int cols) {
        puzzlePnl.setLayout(new GridBagLayout());
